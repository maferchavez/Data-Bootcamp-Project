...
class GithubToS3Operator (BaseOperator):
    #Define the parameters - what unput is needed for the logic to execute
    """
        ...
        if self.github_object not in ('members', 
            ...
            if self.github_repo == 'all':
                repos = [repo['name'] for repo in
                         self.paginate_data(g, self.methodMapper('repositories'))]
                for repo in repos:
            ...
            output = self.retrieve_data(g, repo= self.github_repo)
        output = '\n'.join([json.dumps(flatten(record)) for record in output]) #json.dumps() converts python object into a json string
        s3.load_string(
            string_data = output,
            key = self.s3_key,
    ...
    def retrieve_data(self, g, repo=None):
        """
        This method builds the endpoint and passes it into the "paginate_data" method. It is wrapped in a
        "try/except" in the event that an HTTP Error is thrown.
        This can happen when making a request to a page that does not yet exist (e.g. requesting commits from a
    ...
    def paginate_data(self, g, endpoint):
        ...
        It retrieves 100 at a time and continues to make subsequent requests until it retrieves less than 100 records.
        """
        output = []
    ...
    def methodMapper(self, github_object, org=None, repo=None):
        """
        This method maps the desired object to the relevant endpoint according to v3 of the Github API.
        """
    ...
    def filterMapper(self, record):
        """
        This process strips out unnecessary objects (i.e. ones that are duplicated in other core objects).
        Example: a commit returns all the same user information for each commit as already returned the members endpoint).
        In most cases, id for these striped objects are kept for reference although multiple values can be added to the array.
        Labels is currently returned as an array of dicts.
        When flattened, this can cause an undo amount of columns with the naming convention labels_0_name,
        labels_1_name, etc. Until a better data model can be determined (possibly putting labels in their own table)
        these fields are striped out entirely.
        In situations where there are no desired retention fields, "retained" should be set to "None".
        """
        ...
        def process(record, mapping):
            ...
            There are a number of checks throughout as the specified filtered object and desired retained fields will not always exist in each
            record.
            """
